...
var settingSpecs = []settingSpec{
	...
	{Namespace: "circuit", Key: "channel_circuit_open_seconds", ValueType: "int", Default: "60", Description: "熔断持续时间(秒)",
		Validate: integerValue()},
	// 用户自有渠道（BYOK）配置
	{Namespace: "chat", Key: "user_upstream_enabled", ValueType: "bool", Default: "false", Description: "全局开关：是否允许用户添加自有上游渠道（BYOK）",
		Validate: boolValue(), Apply: applyField(func(c *config.Config) *bool { return &c.UserUpstreamEnabled }, toBool)},
	{Namespace: "chat", Key: "user_upstream_billing_mode", ValueType: "string", Default: "statistics_only", Description: "用户渠道计费策略：disabled=禁用功能 | statistics_only=仅统计不扣费 | platform_pricing=按平台价格扣费",
		Validate: oneOf("disabled", "statistics_only", "platform_pricing"), Apply: applyField(func(c *config.Config) *string { return &c.UserUpstreamBillingMode }, trimmedText)},
	{Namespace: "chat", Key: "user_upstream_quota_limit", ValueType: "int", Default: "3", Description: "单用户最多创建上游数量，0表示无限制",
		Validate: integerValue(), Apply: applyField(func(c *config.Config) *int { return &c.UserUpstreamQuotaLimit }, toInt)},
	{Namespace: "chat", Key: "user_upstream_require_approval", ValueType: "bool", Default: "true", Description: "用户创建渠道是否需要管理员审批",
		Validate: boolValue(), Apply: applyField(func(c *config.Config) *bool { return &c.UserUpstreamRequireApproval }, toBool)},
	{Namespace: "file", Key: "user_embedding_enabled", ValueType: "bool", Default: "false", Description: "是否准入用户通过本人渠道配置Embedding",
		Validate: boolValue(), Apply: applyField(func(c *config.Config) *bool { return &c.UserEmbeddingEnabled }, toBool)},
	{Namespace: "chat", Key: "user_rag_enabled", ValueType: "bool", Default: "false", Description: "是否准入用户和项目使用自有RAG",
		Validate: boolValue(), Apply: applyField(func(c *config.Config) *bool { return &c.UserRAGEnabled }, toBool)},
	{Namespace: "file", Key: "user_embedding_profile_limit", ValueType: "int", Default: "3", Description: "单用户Embedding Profile上限",
		Validate: integerValue(), Apply: applyField(func(c *config.Config) *int { return &c.UserEmbeddingProfileLimit }, toInt)},
	{Namespace: "file", Key: "user_embedding_allowed_protocols", ValueType: "string", Default: "openai", Description: "允许的用户Embedding协议，逗号分隔",
		Validate: maxLength(255), Apply: applyField(func(c *config.Config) *string { return &c.UserEmbeddingAllowedProtocols }, trimmedText)},
	{Namespace: "file", Key: "user_embedding_min_dimensions", ValueType: "int", Default: "64", Description: "用户Embedding最小维度",
		Validate: integerValue(), Apply: applyField(func(c *config.Config) *int { return &c.UserEmbeddingMinDimensions }, toInt)},
